...
scopepod_raw<-st_as_sf(copepods_raw,coords=c("longitude", "latitude"),
                       crs=4326)
```
NOTE: 
...
```{r  mapping}
plot(scopepod_raw["richness_raw"])
```
...
In this module we will use "tmap". "tmap" works similarly to "ggplot2" in that we build and add on layers. Here we only have one layer from sdcopepod_raw. We declare the layer with "tm_shape() (in this case scopepod_raw), then the plot type with the following command.
```{r using "tmap"}
tm_shape(scopepod_raw)+
...
```{r "tmap_save}
tmap_save("../output", filename= "Richness-Map.png",
