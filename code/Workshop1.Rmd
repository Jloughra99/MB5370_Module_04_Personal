...
Can plot the data using ("data=mpg") 
Put the size of the engine ("displ") on the x axis
Put the fuel efficiency ("hwy") on the y axis 
```{r}
ggplot(data=mpg) + 
  geom_point(mapping=aes(x=displ,y=hwy))
```
#Next section from "R for data science" textbook 
The plot shows a negative relationship between engine size (displ) and fuel 
efficiency (hwy). In other words, cars with big engines use more fuel. What does 
this say about fuel efficiency and engine size?

When the cars engine size increases the fuel efficiency decreases by 
approximately double. 

"ggplot()"makes co-ordinate system that you can add layers to. The first 
...
This is the graphing template and you can use this to help develop "ggplot" 
plots. ggplot(data=<DATA>) +
  <GEOM_FUNCTIO>(mapping=aes(<MAPPINGS>))
  
Your "ggplot" code will follow a generic template seen below. You will
ALWAYS:
1)Call "ggplot"
2)Input data
3)Provide a "geom" function with a collection of mappings

Learning the template will help to realize what EXACTLY needs to go into a 
"ggplot2"and how to offer input. Let's revisit this First make an empty 
"ggplot()" that simply provides the co-ordinate system that that the remainder 
of the "ggplot2" arguments can be mapped to. Try it. 

What happens if we just try to run ggplot on its own?
This just created a plot window and nothing else 
```{r}
ggplot()
```
...
ggplot(data=mpg)
```
This also just returned an empty plot 
...
1)A geom 
2)Aesthetics.
Can convey information about your data by mapping aesthetics in your plot to 
...
```
Above have mapped class to colour but can also map to the size aesthetic in the
same way.NOTE: Don't plot a variable which is not continuous to a continuous 
aesthetic like size.
...
Here alpha is what dictates the transparency. If you try a simple number like 
...
#Next section is from ggplot2 alpha reference material 
...
```
There are a range of manual aesthetics that can be set to manually change 
appearance of plot:
...
When I tried this a ggplot was produced where data points were separated into 
teal and red colours based on whether their fuel efficiency was above 5 miles/L. 
If this was true the data point was teal and if this was false the data point 
was red. See below.
```{r}
ggplot(data=mpg) +
...
```
#Troubleshooting 
There were minor errors that appeared in the code from this point on. I will
show both the wrong code and my fixed version.

Below is a purposeful mistake with the corrected version directly below it in a 
chunk. IN this code the "+" should be in top line.
...
It is common practice to break single complex plots into many sub plots (panels).
This allows for development of separate plots for a range of reasons. In
"ggplot" this is done using "facets". To "facet" a plot using 1 variable, use "facet_wrap()" and "~"dictates which variable you want to subset data with. 
...
```
#Facet_grid
If you want to more than one variable use "facet_grid()". Here need two variables 
using "~" to split them up. A separate facet for each combination of drive-type 
(e.g. 4WD) * number of cylinders
```{r}
ggplot(data=mpg) + 
  geom_point(mapping=aes(x=displ,y=hwy)) + 
  facet_grid(drv~cyl)
...
Use a "." if you don't want to facet in row or column dimensions. See what
happens below 
ggplot(data = mpg) +
  geom_point(mapping = aes(x=displ,y=hwy)) +
  facet_grid(.~cyl)
...
```
and this next chunk of code you can allow scales to vary across the panels with 
...
However, ggplot2 can use a variety of geom objects to represent the data. Here, 
we might want to use bar plots, line charts, boxplots and so on. Well we can 
handle this issue in ggplot directly using a different geom to plot the same 
data. Here, instead of plotting points, we will use a smooth line. 
```{r}
ggplot(data=mpg) + 
  geom_smooth(mapping=aes(x=displ,y=hwy))
```
#Question:Why don't we use geom_line() here? What would that look like?
We don't use "geom_line()" here because this package creates line plots and adds 
a line that goes through connecting dots. See below.
```{r}
ggplot(data=mpg) + 
...
```
Question: How does R work out where to plot the line??? Can use the chunk output
to inform us. Can also use the help menu.
...
mappings to its argument making them GLOBAL mappings when applied to every "geom"
A better way...
```{r}
...
colour for the overall relationship (geom_smooth). Neat!

We are Specifying further using class=("subcompact")
```{r}
ggplot(data=mpg,mapping=aes(x=displ,y=hwy)) + 
...
```
#Bar plot
#Next part straight from textbook
...
```
Algorithm to calculate the new values for a graph is called a "stat" and
...
On the "y" axis we are plotting cut 
...
Can also use the "stat_summary" argument to show more about transformations
```{r}
ggplot(data=diamonds) + 
  stat_summary(
...
```
Question: Does anyone notice anything different in the colour scheme of this 
plot?
...
in 4 ways:
1)"Identify" (raw data)
Using "position="identity"" able you to place each object EXACTLY where it 
in the context of the graph. This is vital for chart points (scatter) but does
not work for bar plots as too much information. So in this case need to change
the bar aesthetic 
```{r}
...
position="fill" works like stacking, but makes each set of stacked bars the same
height.
```{r}
ggplot(data=diamonds) + 
...
#Position="dodge" 
3)"Dodge" (doesn't put things on top of each other)
"Position=dodge"" places overlapping objects directly beside each other places 
